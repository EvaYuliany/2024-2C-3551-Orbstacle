...
public class Player : IDisposable {
    ...
    // Materiales definidos previamente
    ...
    // Propiedad que guarda la cámara para obtener la posición (suponiendo que se pase en algún momento)
    public Matrix ViewMatrix { get; set; } // Esta propiedad debe ser establecida desde la cámara
    ...
    public void Draw(Effect Effect, Matrix projectionMatrix) {
try {
        // Calcula la matriz WorldViewProjection
        Matrix worldViewProjection = World * ViewMatrix * projectionMatrix;
        Effect.Parameters["World"].SetValue(World);
        Effect.Parameters["WorldViewProjection"].SetValue(worldViewProjection);

        // Calcula la matriz inversa y transpuesta de World
        Matrix inverseTransposeWorld = Matrix.Transpose(Matrix.Invert(World));
        Effect.Parameters["InverseTransposeWorldMatrix"].SetValue(inverseTransposeWorld);

        // Calcula la posición de la cámara a partir de la ViewMatrix
        Vector3 cameraPosition = -Vector3.Transform(Vector3.Zero, ViewMatrix);
        Effect.Parameters["eyePosition"].SetValue(cameraPosition);

        // Dibuja el modelo
        Model.Draw(Effect);
    } catch (ArgumentException e) {
        Console.WriteLine("Error al pasar parámetros al shader: " + e.Message);
        Console.WriteLine("Pila de llamadas: " + e.StackTrace);
        // Aquí puedes imprimir el valor de las matrices para ver si son válidas
        Console.WriteLine("World: " + World);
        Console.WriteLine("WorldViewProjection: " + (World * ViewMatrix * projectionMatrix));
        Console.WriteLine("InverseTransposeWorldMatrix: " + Matrix.Transpose(Matrix.Invert(World)));
        Console.WriteLine("eyePosition: " + (-Vector3.Transform(Vector3.Zero, ViewMatrix)));
    }
}
